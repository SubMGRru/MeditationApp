...
public class AuthService {
  ...
  public static void anonymousAuth(Context context, android.view.View viewBox) {
    ...
    Snackbar snackbar = Snackbar.make(viewBox, context.getString(R.string.snackbar_connectionAwait),
        Snackbar.LENGTH_INDEFINITE);
    snackbar.show();

    StringRequest stringRequest = new StringRequest(Request.Method.GET,
        context.getString(R.string.api_server) + "/service/auth_anonymousAccess",
        new Response.Listener<String>() {
          ...
          public void onErrorResponse(VolleyError error) {
            ...
            if (error instanceof TimeoutError || error instanceof ServerError
              ...
              context.startActivity(new Intent(context, NoConnection.class).putExtra("reasonState",
                  "unavailable_server"));
              ((Activity) context).finish();
            } else if (error instanceof NetworkError || error instanceof NoConnectionError) {
              context.startActivity(
                  new Intent(context, NoConnection.class).putExtra("reasonState", "no_connection"));
              ((Activity) context).finish();
            } else {
